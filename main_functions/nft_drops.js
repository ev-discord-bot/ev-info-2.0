...
window.addEventListener('load', async () => {
    ...
    try {
        ...
        for (const nftDrop of nftDropData) {
            ...
            const nftDropContent = `<div class="grid grid-rows-1 grid-flow-col gap-5 items-center justify-center p-1 scale-[0.9] w-auto"><div class="rounded-lg p-3 flex items-center justify-center w-auto"><div class="flex items-center"><img src="${tokenimg}" alt="Sol Logo" class="size-20 mr-2 h-auto"></div><div class="nft-drop-info text-sm ml-4 w-full"><p>${nftDrop.title}</p><p>${nftDrop.field_tier} Tier</p><p>${nftDrop.field_drop_chance} Chance</p><p>${nftDrop.field_quantity_left} Left</p></div></div></div>`;
